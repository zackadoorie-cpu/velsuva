...
3. Add `mission_board_unlock` to your `mod_load_order.txt` **after** `Darktide Mod Framework` (example file included at repo root). Darktide only loads mods listed in that file, and it must contain **only** mod folder names (one per line, no comments or prefixes).
...
- Match the folder names exactly in `mod_load_order.txt` (for example, `Darktide Mod Framework` must match the installed framework folder name, and `mission_board_unlock` must match the mod folder name). Remove any comment lines or prefixes.
